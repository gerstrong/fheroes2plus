...
int Game::HighScores(bool fill)
{
	...
	const u32 cdays = Settings::Get().CampainDays();
	...
	std::cout  << cdays;
	std::cout  << " Дни кампании \n";

	std::cout  << Settings::Get().GameType ();
	std::cout  << " Тип игры \n";
    ...
    if(le.MouseClickLeft(buttonCampain) || HotKeyCloseWindow) return HIGHSCORESCAMPAIN;
...
#define HGSC_ID	0xF1F3
#define HGSC_MAX	10

struct hgsc_t
{
    hgsc_t() : days(0), rating(0) {};

    bool operator== (const hgsc_t &) const;
...
StreamBase & operator<< (StreamBase & msg, const hgsc_t & hgsc)
{
    return msg << hgsc.player << hgsc.land << hgsc.localtime << hgsc.days << hgsc.rating;
}

StreamBase & operator>> (StreamBase & msg, hgsc_t & hgsc)
{
    return msg >> hgsc.player >> hgsc.land >> hgsc.localtime >> hgsc.days >> hgsc.rating;
}

bool hgsc_t::operator== (const hgsc_t & h) const
{
    return player == h.player && land == h.land && days == h.days;
}

bool RatingSortCampian(const hgsc_t & h1, const hgsc_t & h2)
{
    return h1.rating > h2.rating;
}
...
public:
    ...
    bool Load(const std::string &);
    bool Save(const std::string &);
    void ScoreRegistryCampain(const std::string &, const std::string &, u32, u32);
    void RedrawList(s32, s32);
private:
    std::vector<hgsc_t> list;
};

bool HGSCData::Load(const std::string & fn)
{
    ZStreamFile hdata;
    if(! hdata.read(fn)) return false;
...
bool HGSCData::Save(const std::string & fn)
{
    ZStreamFile hdata;
    hdata.setbigendian(true);
    hdata << static_cast<u16>(HGSC_ID) << list;
    if(hdata.fail() || ! hdata.write(fn)) return false;

    return true;
}

void HGSCData::ScoreRegistryCampain(const std::string & p, const std::string & m, u32 r, u32 s)
{
    hgsc_t h;
    ...
    {
	list.push_back(h);
	std::sort(list.begin(), list.end(), RatingSortCampian);
	if(list.size() > HGSC_MAX) list.resize(HGSC_MAX);
    }
}

void HGSCData::RedrawList(s32 ox, s32 oy)
{
    const Settings & conf = Settings::Get();
    ...
    const Sprite &head = AGG::GetICN(ICN::HISCORE, 7);
    if(conf.QVGA())
	head.Blit(ox + 25, oy + 15);
    else
	head.Blit(ox + 50, oy + 31);
    ...
    std::vector<hgsc_t>::const_iterator it1 = list.begin();
    std::vector<hgsc_t>::const_iterator it2 = list.end();

    Text text;
    text.Set(conf.QVGA() ? Font::SMALL : Font::BIG);

    for(; it1 != it2 && (it1 - list.begin() < HGSC_MAX); ++it1)
    {
	const hgsc_t & hgsc = *it1;

	text.Set(hgsc.player);
	text.Blit(ox + (conf.QVGA() ? 45 : 88), oy + (conf.QVGA() ? 33 : 70));

	text.Set(hgsc.land);
	text.Blit(ox + (conf.QVGA() ? 170 : 260), oy + (conf.QVGA() ? 33 : 70));

	text.Set(GetString(hgsc.days));
	text.Blit(ox + (conf.QVGA() ? 250 : 455), oy + (conf.QVGA() ? 33 : 70));

	//text.Set(GetString(hgsc.rating));
	//text.Blit(ox + (conf.QVGA() ? 270 : 480), oy + (conf.QVGA() ? 33 : 70));
...
int Game::HighScoresCampain(bool fill)
{
	...
	cursor.Hide();
	if(player.empty()) player = _("Unknown Hero");
	hgsc.ScoreRegistryCampain(player, Settings::Get().CurrentFileInfo().name, cdays, rating);
    //hgsc.ScoreRegistryCampain(player, "ETO TESTEST", 100,999);
