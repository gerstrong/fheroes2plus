...
{
	std::cout  << world.CountDay();
	std::cout  << " Дни CountDay при проверке \n";
 ...
 //  conf.SetGameType(Game::TYPE_CAMPAIGN);
  ...
  // Загрузка карты компании  
	...
	Settings::Get().SetCampainDays(0);
//	std::cout  << Settings::Get().CampainDays();
//	std::cout  << " Дни камапании при первом запуске игры \n";
...
//nameFileout.open("files/campain/t800.txt");
...
std::cout  << world.CountDay() << " Дни CountDays  \n";	
Settings::Get().SetCampainDays(Settings::Get().CampainDays() + world.CountDay());
std::cout  << Settings::Get().CampainDays() << " Дни CampainDays  \n";	
const u32 cndays = Settings::Get().CampainDays();
	...
	//std::cout << search;
//    std::string fname = System::ConcatePath(path,"t800.txt");
//	std::cout << path << " Путь\n";
//	std::cout << fname << " Имя с путем\n";
	//std::cout << search;

         
	std::cout << savname << " Имя карты\n";
    std::string listname = StringLower(conf.MapsFile().substr(0,5)) + ".txt";
    std::cout << listname << " Имя списка\n";
    listname = System::ConcatePath(path,listname);
	std::cout << listname << " Полное Имя списка\n";
  
    std::ifstream scenarioListFile;
    std::string scenarioFileName;

	//Settings & conf = Settings::Get();
    //std::string search = conf.MapsName();
	std::cout << savname << " \n";
	...
	std::cout << file << " Имя файла\n";
	
//if(file.empty()) return Game::HIGHSCORESCAMPAIN;	
if(file.empty() || !Game::Load(file)) return Game::HIGHSCORESCAMPAIN;
//if(!Game::Load(file)) return MAINMENU;
//Settings::Get().SetCampainDays(100);

//const u32 rating = GetGameOverScores();
//days = world.CountDay();
//cdays = Settings::Get().CampainDays();
//cdays = cdays+days;
Settings::Get().SetCampainDays(cndays);
std::cout  << Settings::Get().CampainDays() << " Дни CampainDays  \n";
return STARTGAME;
